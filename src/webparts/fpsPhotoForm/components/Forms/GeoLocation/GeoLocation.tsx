...
const GpsLocationForm: React.FC<IFpsGpsLocationFormProps> = (props) => {
  ...
  const handleGetLocation = async (): Promise<void> => {
    const locationDetails: IFpsGeolocationPosition = await getGeoLocation( debugMode );
    if ( debugMode ) alert(`debugMode GpsLocationForm ~ 33: ${JSON.stringify( locationDetails )}`);
  ...
  const handleDebugChange = (checked: boolean): void => {
